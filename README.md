## Kaushik Rajan

Engineer and computer scientist with a decade of experience in applied science (including 4 years at Amazon). Building applied AI products informed by causal inference, game theory, and multi-agent systems research.

MS Computer Science (NC State) · [kaushikrajan.me](https://kaushikrajan.me) · Open to research roles & collaborations

---

### Research Interests

- **Causal Inference & Attribution** — Shapley-value decomposition, causal discovery, interpretable AI
- **Game Theory & Mechanism Design** — Coalition manipulation bounds, public goods games, strategic interaction
- **Multi-Agent Reinforcement Learning** — Cooperative/competitive dynamics, self-play, emergent behavior
- **Agentic Systems** — Tool-augmented reasoning, autonomous multi-agent workflows

---

### At a Glance

<p align="center">
  <img src="https://img.shields.io/badge/Upstream_PRs_Merged-3-2563eb?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Research_Projects-4-059669?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Publications-5+-7c3aed?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Interactive_Demos-7-d97706?style=for-the-badge" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=kvr06-ai&theme=tokyonight&hide_border=true" />
</p>

---

### Open Source Contributions

Contributing to research-grade open source in causal inference, game theory, and ML infrastructure:

**Merged:** [dspy](https://github.com/stanfordnlp/dspy) (Stanford, 33K ⭐) · [optuna](https://github.com/optuna/optuna) (13.7K ⭐) · [statsmodels](https://github.com/statsmodels/statsmodels) (11K ⭐)

**In Review:** [open_spiel](https://github.com/google-deepmind/open_spiel) (DeepMind, 5.1K ⭐) · [dowhy](https://github.com/py-why/dowhy) (PyWhy, 8K ⭐) · [spaCy](https://github.com/explosion/spaCy) (33K ⭐) · [diffusers](https://github.com/huggingface/diffusers) (HuggingFace, 33K ⭐) · [instructor](https://github.com/instructor-ai/instructor) (12.5K ⭐)

---

### Research Projects

| Project | Description |
|:--------|:------------|
| [**cola-manipulation-bound**](https://github.com/kvr06-ai/cola-manipulation-bound) | Analytic upper bound on pool manipulation in the COLA draft mechanism |
| [**shapley-hri-attribution**](https://github.com/kvr06-ai/shapley-hri-attribution) | Causal credit assignment in human-robot interaction via Shapley values · [Demo](https://kvr06-ai.github.io/shapley-hri-attribution/) |
| [**trust-based-public-goods-game**](https://github.com/kvr06-ai/trust-based-public-goods-game) | Multi-agent public goods simulation with trust dynamics and RL |
| [**self-play-reasoning**](https://github.com/kvr06-ai/self-play-reasoning) | Implementation of SPIRAL paper principles for advanced AI reasoning |

### Interactive Explorations

Educational demos exploring ML/RL concepts through interactive visualization:

[gradient-descent](https://github.com/kvr06-ai/gradient-descent) · [object-detection](https://github.com/kvr06-ai/object-detection) · [multi-agent-rl-sim](https://github.com/kvr06-ai/multi-agent-rl-sim) · [multi-armed-bandits-explorer](https://github.com/kvr06-ai/multi-armed-bandits-explorer) · [rl-pong](https://github.com/kvr06-ai/rl-pong) · [rl-capture-the-flag](https://github.com/kvr06-ai/rl-capture-the-flag)

---

### Publications & Writing

[![SSRN](https://img.shields.io/badge/SSRN-154881?style=flat&logo=ssrn&logoColor=white)](https://papers.ssrn.com/sol3/cf_dev/AbsByAuth.cfm?per_id=7017868)
[![Google Scholar](https://img.shields.io/badge/Google_Scholar-4285F4?style=flat&logo=googlescholar&logoColor=white)](https://scholar.google.com/citations?user=DU_lorIAAAAJ&hl=en)
[![Medium](https://img.shields.io/badge/Medium-000?style=flat&logo=medium&logoColor=white)](https://kaushikvr06.medium.com/)
[![Website](https://img.shields.io/badge/kaushikrajan.me-000?style=flat&logo=safari&logoColor=white)](https://kaushikrajan.me)

---

*Open to research roles (PhD, Research Scientist, RA) and collaborative projects. Reach out via [website](https://kaushikrajan.me).*
